...
number of buildings, by any amount (the amounts can be different for different buildings).
...
class Solution:
    def max_increase_keeping_skyline(self, grid):
        ...
        :rtype: int
        """
        r, c, output = len(grid), len(grid[0]), 0
        for row in range(r):
            for col in range(c):
                row_max = max(grid[row])
                trans_grid = [*zip(*grid)]
                col_max = max(trans_grid[col])
                allowed_height = min(row_max, col_max)
                output += abs(allowed_height - grid[row][col])
        return output