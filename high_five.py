...
class Solution:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        student_records, output = {}, []
        for item in items:
            if item[0] in student_records:
                student_records[item[0]].append(item[1])
            else:
                student_records[item[0]] = [item[1]]
        
        for student in student_records:
            student_records[student].sort(reverse=True)
            avg = int(sum(student_records[student][:5]) / 5)
