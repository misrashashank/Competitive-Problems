...
class Solution:
    def roman_to_int(self, s):
        ...
        for index in range(roman_len):
            if index in skip_list:
                continue

            if (index + 1) in range(roman_len) and ((s[index] == "I" and s[index + 1] in ("V", "X"))
                or (s[index] == "X" and s[index + 1] in ("L", "C"))
                   or (s[index] == "C" and s[index + 1] in ("D", "M"))):
                sum += roman_dict[s[index + 1]] - roman_dict[s[index]]
                skip_list.append(index + 1)
            else:
                sum += roman_dict[s[index]]
        return sum